...
#Область ктСозданиеЗадачи  

//↓↓↓ Китэк-Систем -- ЕвсеенкоРС -- 31.10.2023 ↓↓↓ 

Процедура ЗаписатьОбъект(Об)
...
Процедура СоздатьИсходящееСообщение(ПисьмоОбъект, НомерЗадачи)
	
	ШаблонСообщения = Справочники.ШаблоныПисем.НайтиПоНаименованию("Ответ на заявку").ПолучитьОбъект();
		
	ИсходящееСообщение = Документы.ИсходящееПисьмо.СоздатьДокумент();
	ИсходящееСообщение.Тема = ШаблонСообщения.Тема;
	
	ИсходящееСообщение.Дата = ПисьмоОбъект.Дата; 
	ИсходящееСообщение.ПодготовленоКОтправке = ТекущаяДата();
	ИсходящееСообщение.УчетнаяЗапись = ПисьмоОбъект.УчетнаяЗапись;	
	ИсходящееСообщение.ТипТекста = ШаблонСообщения.ТипТекста;	 	
	ИсходящееСообщение.Важность = ШаблонСообщения.Важность;  
	
	ПолучательПисьма = ИсходящееСообщение.ПолучателиПисьма.Добавить();
	ПолучательПисьма.Адресат = ПисьмоОбъект.ОтправительАдресат;   	
	ИсходящееСообщение.ПолучателиПисьмаСтрокой = ВстроеннаяПочтаСервер.ТаблицаПолучателейВСтроку(
			ИсходящееСообщение.ПолучателиПисьма);
 		
	ТекстПисьмаДляЗаписиHTML = СтрЗаменить(
			ШаблонСообщения.ТекстШаблонаПисьмаHTML, "&amp;НомерЗадачи", НомерЗадачи);
			
	ИсходящееСообщение.ДополнительныеСвойства.Вставить("ТекстПисьма", ТекстПисьмаДляЗаписиHTML);  
	
	ЗаписатьОбъект(ИсходящееСообщение); 

КонецПроцедуры 

Функция ПолучитьНомерЗадачи(Строка)
	
	ПервыйСимвол = СтрНайти(Строка, " ", НаправлениеПоиска.СКонца) + 1;
	КоличествоСимволов = СтрДлина(Строка) - ПервыйСимвол + 1;
	НомерЗадачи = Сред(Строка, ПервыйСимвол, КоличествоСимволов);  
	...
	НомерЗадачи = Формат(НомерЗадачи, "ЧЦ=9; ЧДЦ=0; ЧВН=; ЧГ=0");
	
	Возврат Строка(НомерЗадачи);	
	
КонецФункции	      

Функция ПолучитьЗаказчика(ПисьмоОбъект)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЗаказчикиКонтактнаяИнформация.Ссылка КАК Заказчик
	|ПОМЕСТИТЬ ВТ_Заказчики
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ВТ_КонтактныеЛица.КонтактноеЛицоЗаказчика, НЕОПРЕДЕЛЕНО) КАК КонтактноеЛицоЗаказчика,
	|	ЕСТЬNULL(ЕСТЬNULL(ВТ_КонтактныеЛица.Заказчик, ВТ_Заказчики.Заказчик), НЕОПРЕДЕЛЕНО) КАК Заказчик
	|ИЗ
	|	ВТ_КонтактныеЛица КАК ВТ_КонтактныеЛица
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВТ_Заказчики КАК ВТ_Заказчики
	|		ПО (ИСТИНА)";
	
	Запрос.УстановитьПараметр("АдресЭП", ПисьмоОбъект.ОтправительАдресат.Адрес);
	
	Выборка = Запрос.Выполнить().Выбрать();

	Заказчик = Новый Структура("Заказчик, КонтактноеЛицоЗаказчика", Неопределено, Неопределено); 
	
	Пока Выборка.Следующий() Цикл
		Заказчик.Заказчик = Выборка.Заказчик;
		Заказчик.КонтактноеЛицоЗаказчика = Выборка.КонтактноеЛицоЗаказчика;
	КонецЦикла;
...
Процедура СоздатьЗадачу(ДокументЗадача, ТекстПисьма, ПисьмоОбъект)
	
	ДокументЗадача.Наименование = ПисьмоОбъект.Тема;
	ДокументЗадача.Описание = ТекстПисьма;
	ДокументЗадача.Дата = ПисьмоОбъект.Дата;
	ДокументЗадача.Статус = Перечисления.СтатусыЗадач.ктНеобработана; 
	
	СтруктураЗаказчик = ПолучитьЗаказчика(ПисьмоОбъект);
	
	ДокументЗадача.Заказчик = СтруктураЗаказчик.Заказчик;	
	ДокументЗадача.КонтактноеЛицоЗаказчика = СтруктураЗаказчик.КонтактноеЛицоЗаказчика; 
	
	ЗаписатьОбъект(ДокументЗадача); 
		
КонецПроцедуры      

Процедура ДополнитьЗадачу(ДокументЗадача, ТекстПисьма, ПисьмоОбъект)
	
	ДокументЗадача.Описание = СтрШаблон("%1%2%3%4%5%6",
			ДокументЗадача.Описание, Символы.ПС, Символы.ПС, ПисьмоОбъект.ДатаПолучения, Символы.ПС, ТекстПисьма); 
			
	ЗаписатьОбъект(ДокументЗадача);
	
КонецПроцедуры   

Процедура СкопироватьФайлы(Задача, ПисьмоОбъект)
	...
	Пока Выборка.Следующий() Цикл
		КопияФайла = Выборка.Ссылка.Скопировать();
		КопияФайла.ВладелецФайла = Задача;
		ЗаписатьОбъект(КопияФайла);	
	КонецЦикла;
	
КонецПроцедуры    

Процедура ОбработатьЗадачуИСоздатьСообщение(ПисьмоОбъект)
	
	ТекстПисьма = ПисьмоОбъект.ПолучитьТекстовоеПредставлениеСодержанияПисьма();
	ТекстПисьма =  СокрЛП(ТекстПисьма);

	НомерЗадачи = ПолучитьНомерЗадачи(ПисьмоОбъект.Тема);    
	
	Задача = Документы.Задачи.НайтиПоНомеру(НомерЗадачи, ПисьмоОбъект.Дата); 
	
	Если Не ЗначениеЗаполнено(Задача) Тогда
		Задача = Документы.Задачи.НайтиПоНомеру(НомерЗадачи, ДобавитьМесяц(ПисьмоОбъект.Дата, -12));	
	КонецЕсли;
	
	ПисьмоОбъект.ТекстПисьмаHTMLХранилище.Получить();
	НоваяЗадача = Не ЗначениеЗаполнено(Задача);
	Если НоваяЗадача Тогда
		ДокументЗадача = Документы.Задачи.СоздатьДокумент();
		ДокументЗадача.ОбменДанными.Загрузка = Истина; //редакция КИТЭК ЧехловНИ 19.04.2024
		СоздатьЗадачу(ДокументЗадача, ТекстПисьма, ПисьмоОбъект);	  
		
		ПисьмоОбъект.Тема = СтрШаблон("%1 %2", ПисьмоОбъект.Тема, ДокументЗадача.Номер);
		//+++++++++СоздатьИсходящееСообщение(ПисьмоОбъект, ДокументЗадача.Номер);
	Иначе
		ДокументЗадача = Задача.ПолучитьОбъект();
		ДополнитьЗадачу(ДокументЗадача, ТекстПисьма, ПисьмоОбъект);	
	КонецЕсли; 
		
	СкопироватьФайлы(ДокументЗадача.Ссылка, ПисьмоОбъект);	
...
Функция ПолучитьИОтправитьПисьма() Экспорт
	...
	//↓↓↓ Китэк-Систем -- ЕвсеенкоРС -- 31.10.2023 ↓↓↓			→→→		Поменял процедуры местами 			
...
Функция ЗаписатьВходящееПисьмо(Сообщение, УчетнаяЗапись, 
	...
	Попытка
		...
		//↓↓↓ Китэк-Систем -- ЕвсеенкоРС -- 31.10.2023 ↓↓↓ 
		ОбработатьЗадачуИСоздатьСообщение(ПисьмоОбъект); 
		//↑↑↑ Китэк-Систем -- ЕвсеенкоРС -- 31.10.2023 ↑↑↑
