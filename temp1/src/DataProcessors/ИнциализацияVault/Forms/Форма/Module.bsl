...
Функция ПолучитьТекущийТокен() Экспорт
    // Безопасная проверка токена в сеансе
    Если ПараметрыСеанса.Свойство("Токен") Тогда
        Возврат ПараметрыСеанса.Токен;
    Иначе
        // Возвращаем пустую строку — заглушка
        Возврат "";
    КонецЕсли;
КонецФункции
...
Процедура ПриОткрытии(Отказ, СтандартнаяОбработка)
    ...
    // Если токен пуст — просто сообщаем, но не падаем
    Если ПустаяСтрока(ПараметрыVault.Токен) Тогда
        Сообщить("Токен пока не установлен. Вы можете его сгенерировать или вставить вручную.");
    КонецЕсли;
КонецПроцедуры
