...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СуммаКорректируемая.Видимость = Объект.ОплатаПоТарифу;
...
#КонецОбласти
&НаКлиенте
Процедура ВыгрузкаСтрокойПриИзменении(Элемент)
	Элементы.СуммаКорректируемая.Видимость = Истина;
	Если НЕ ЗначениеЗаполнено(Объект.СуммаКорректируемая) Тогда
		ОБъект.СуммаКорректируемая = Объект.Сумма;
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьJSONСтрокуРаботы()
	...
	Запрос = Новый Запрос;
	Если Объект.ОплатаПоТарифу Тогда
	Запрос.Текст = "ВЫБРАТЬ
	|	ЕСТЬNULL(Трудозатраты.Заказчик.ИНН, 0) КАК КонтрагентИНН,
	|	ЕСТЬNULL(Трудозатраты.Организация.ИНН, 0) КАК ОрганизацияИНН,
	|	""Сопровождение по договору"" КАК Наименование,
	|	Трудозатраты.Количество КАК Количество,
	|	Трудозатраты.СуммаКорректируемая КАК Цена,
	|	Трудозатраты.СуммаКорректируемая КАК Сумма,
