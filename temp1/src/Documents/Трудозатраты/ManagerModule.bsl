...
Процедура ЗапросыФормированияТрудозатрат(Запрос, ЗапросИзДокумента)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	               ...
	               |	И (Задачи.ДокументОснование = ЗНАЧЕНИЕ(Документ.Задачи.ПустаяСсылка) ИЛИ (Задачи.ДокументОснование <> ЗНАЧЕНИЕ(Документ.Задачи.ПустаяСсылка) И Задачи.ВключатьВТрудозатраты))
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	Задачи.Исполнитель,
...
Функция ЗаполнитьТабличнуюЧасть(Проекты, УчитыватьНевключенныеЗадачи, Дата, Отдел, Заказчик, Трудозатраты, ДатаНачала = Неопределено, ДатаОкончания = Неопределено) Экспорт   //Редакция КИТЭК ЧехловНИ 05.04.2024
	Запрос = новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	Запрос.УстановитьПараметр("НачалоПериода",НачалоМесяца(?(ДатаНачала= Неопределено, Дата, ДатаНачала)));
	//+++ КИТЭК ЧехловНИ 05.04.2024
	Если УчитыватьНевключенныеЗадачи Тогда
		
		ДатаНачала = Дата(0001,01,01);
		Запрос.УстановитьПараметр("НачалоПериода", ДатаНачала);		
		
	КонецЕсли;
	//--- КИТЭК ЧехловНИ 05.04.2024
	Запрос.УстановитьПараметр("КонецПериода",КонецДня(?(ДатаОкончания= Неопределено, Дата, ДатаОкончания)));
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	втЗадачи.Задача.Номер КАК Номер, 
				   //--- КИТЭК ХандожкоМА 19.02.2025  
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.кт_УчтенныеЗадачи КАК кт_УчтенныеЗадачи
	               |		ПО втЗадачи.Задача = кт_УчтенныеЗадачи.Задача
	               |ГДЕ
	               |	кт_УчтенныеЗадачи.Задача ЕСТЬ NULL
	               |	И втЗадачи.Проект В(&Проект)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ЗадачаДатаСтатуса,
	               |	Проект,
	...
	//+++ КИТЭК ЧехловНИ 05.04.2024	11.04.2024
	...
	Иначе
		...
		Сообщить("Выберите хотя бы один проект!");
