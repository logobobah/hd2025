...
Процедура ТокенНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
    // Отключаем встроенное поведение (список справочника)
    СтандартнаяОбработка = Ложь;

    // Создаём диалог открытия файла
    ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
    ДиалогОткрытияФайла.Заголовок = "Выберите файл с токеном";
    // Фильтр — необязательно, можно ограничить по расширению
    ДиалогОткрытияФайла.Фильтр = "Текстовые файлы (*.txt)|*.txt|Все файлы (*.*)|*.*";
    ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
    ДиалогОткрытияФайла.ПолноеИмяФайла = "";

    // Отображаем диалог
    Если ДиалогОткрытияФайла.Выбрать() Тогда
        // Если файл выбран — сохраняем путь в поле
        Объект.Токен = ДиалогОткрытияФайла.ПолноеИмяФайла;
    Иначе
        // Если отмена — очищаем поле
        Объект.Токен = "";
    КонецЕсли;
КонецПроцедуры
